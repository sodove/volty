    ...
    override suspend fun upsert(vehicle: Vehicle) {
        ...
        queries.transaction {
            queries.upsert(
                ...
                cellHighEnabled = if (a.cellHighEnabled) 1L else 0L,
                cellLowEnabled = if (a.cellLowEnabled) 1L else 0L,
                cellDeltaEnabled = if (a.cellDeltaEnabled) 1L else 0L,
                temperatureWarnEnabled = if (a.temperatureWarnEnabled) 1L else 0L,
                temperatureHighEnabled = if (a.temperatureHighEnabled) 1L else 0L,
                socLowEnabled = if (a.socLowEnabled) 1L else 0L,
                socCutoffEnabled = if (a.socCutoffEnabled) 1L else 0L,
...
private fun VehicleRow.toDomain(
    ...
    alertConfig = AlertConfig(
        ...
        cellHighEnabled = cellHighEnabled == 1L,
        cellLowEnabled = cellLowEnabled == 1L,
        cellDeltaEnabled = cellDeltaEnabled == 1L,
        temperatureWarnEnabled = temperatureWarnEnabled == 1L,
        temperatureHighEnabled = temperatureHighEnabled == 1L,
        socLowEnabled = socLowEnabled == 1L,
        socCutoffEnabled = socCutoffEnabled == 1L,
    ...
    dashboardStyle = DashboardStyle.fromPersistedName(dashboardStyle),
