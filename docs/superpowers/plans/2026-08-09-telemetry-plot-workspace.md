...
- [x] **Step 1: Write failing tests** for a union time axis, per-series non-zero range, and normalization of a value to the plot fraction.
- [x] **Step 2: Run the focused test and confirm the missing-helper failure.**
- [x] **Step 3: Implement the smallest pure helpers** and keep unknown/empty series empty.
- [x] **Step 4: Run the focused test and commit** `feat(graph): add overlay plot geometry`.
...
- [x] **Step 1: Render one shared canvas** with colored traces, per-trace ranges, grid, legend, common cursor, and tap-to-time selection.
- [x] **Step 2: Render a complete grouped metric table** with Plot checkboxes, current/selected values, units, and no hidden columns.
- [x] **Step 3: Make portrait and landscape layouts adaptive** using measured width and weights; keep all spacing inset-derived.
- [x] **Step 4: Replace the clipped comparison dialog** with the same plot/table workspace while preserving history actions and callbacks.
- [x] **Step 5: Add localized strings and run the debug compile**, then commit `feat(graph): redesign comparison workspace`.
